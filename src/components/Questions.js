...
import { questionJS } from "../data/Preguntas";
...
export default class Questions extends Component {
  ...
  componentDidMount() {
    ...
    const currentVidas = questionJS[this.state.vidas];
  ...
  handleSubmit = (e) => {
    ...
    if (this.state.answerSelect === this.state.question.correct) {
      ...
      else{console.log(this.state.score)}
  };
    ...
    return (
      <form className="container" onSubmit={this.handleSubmit}>
        ...
        <h2 style={{ textAlign: "center" }}>
          Pregunta
          {this.state.numberQuestion} de {questionJS.length}
        </h2>
      </form>
    );
  }
